...
The benchmark supports two kinds of prediction. The first is sample-level phenotype classification (dysbiotic vs non-dysbiotic). The second, and the main focus of this thesis, is ranking individual protein families by how likely they are to be bioactive in IBD.
...
**Evaluating imbalanced rankers.** The bioactive label is rare, which shapes how I evaluate. Precision–recall takes priority over ROC here, since Saito & Rehmsmeier (2015) show that PR curves are the more informative summary under heavy class imbalance. Remaining uncertainty is then quantified with the non-parametric bootstrap (Efron & Tibshirani 1993).
...
# 4. Bioactive protein-family prioritization

## 4.1 Setup

The goal here is to rank about 1.4 million microbial protein families by how likely they are to be bioactive in IBD, and to beat the published MetaWIBELE priority scores on the same families and labels.
